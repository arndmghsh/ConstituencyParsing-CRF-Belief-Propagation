...
def message_RoottoL(node: MyNode, mu_from_parent):
    ...
    # Prediction
    V = belief.view(1,-1)
    ce_loss += criterion(V, node.true_label)
    # Tags
    if node.true_label == torch.argmax(V, dim=1):
        n_correct +=1
        if len(node.children)==0:
            l_correct+=1
...
train_sentences = encoded_sentences[0:2000]
test_sentences = encoded_sentences[2000:3000]
train_trees = my_trees[0:2000]
test_trees = my_trees[2000:3000]
